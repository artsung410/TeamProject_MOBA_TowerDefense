...
public class ItemAdderButtonWizard : MonoBehaviour
{
    // ###############################################
    //             NAME : Simstealer                      
    ...
    [SerializeField]
    private GameObject wizardDropFieldData;

    private void Start()
    {
        inv = GameObject.FindGameObjectWithTag("Canvas").transform.GetChild(1).GetComponent<Inventory>();
    }
    ...
    public void ItemAdder()
    {
        ...
        {
            if (wizardDropFieldData.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text == inventoryItemList_skill.itemList[i].itemName)
            {
