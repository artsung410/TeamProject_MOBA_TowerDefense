...
public class ImageChangers_small : MonoBehaviourPun
{
    // ###############################################
    //             NAME : Simstealer                      
    ...
    private void Awake()
    {
        ...
        {
            transform.position = LoadingMagager.Instance.loadingCharactorSmallPos[0].position;
            ChangeCharacterImages();
        }

        else
        {
            transform.position = LoadingMagager.Instance.loadingCharactorSmallPos[1].position;
        }
    ...
    {
        smallImage.GetComponent<Image>().sprite = LoadingMagager.Instance.heroImages_small[heroIndex];
    }
}
